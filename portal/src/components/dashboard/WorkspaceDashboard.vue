<template>
  <div class="space-y-6">
    <!-- Header & Filter Bar -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 border border-slate-800 p-4 rounded-xl backdrop-blur-sm">
      <div>
        <h2 class="text-lg font-bold text-slate-100 flex items-center gap-2">
          <LayoutDashboard class="w-5 h-5 text-blue-400" />
          {{ workspaceTitle }}
        </h2>
        <p class="text-xs text-slate-400 mt-0.5">Real-time ERPNext live operational metrics & business analytics</p>
      </div>
      ...
      <div class="flex items-center gap-3">
        <button 
          @click="loadDashboard" 
          :disabled="loading"
          class="flex items-center gap-1.5 px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-medium rounded-lg border border-slate-700 transition-colors disabled:opacity-50"
        >
    ...
    <div v-if="loading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div v-for="i in 4" :key="i" class="h-28 bg-slate-900/60 border border-slate-800 rounded-xl animate-pulse"></div>
    </div>
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div 
          ...
          class="bg-slate-900 border border-slate-800 hover:border-blue-600/50 cursor-pointer rounded-xl p-4 transition-all shadow-sm flex flex-col justify-between group"
        >
          <div class="flex items-center justify-between text-slate-400 mb-2">
            <span class="text-xs font-semibold uppercase tracking-wider text-slate-400 group-hover:text-slate-200 transition-colors">{{ kpi.label }}</span>
            <component :is="getKpiIcon(kpi.color)" class="w-4 h-4" :class="getKpiIconClass(kpi.color)" />
          </div>
          <div>
            <div class="text-2xl font-bold text-slate-100 font-mono tracking-tight group-hover:text-blue-400 transition-colors">
      ...
      <div v-if="dashboard.charts && dashboard.charts.length" class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div 
          ...
          :key="cIdx"
          class="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm flex flex-col justify-between"
        >
          <h3 class="text-xs font-semibold text-slate-300 uppercase tracking-wider mb-4 flex items-center gap-2">
          ...
          <div v-if="chart.data && chart.data.length" class="space-y-3">
            <div v-for="(item, iIdx) in chart.data" :key="iIdx" class="space-y-1">
              <div class="flex items-center justify-between text-xs font-medium">
                <span class="text-slate-300 truncate">{{ item[chart.label_key] || 'Unspecified' }}</span>
                <span class="font-mono text-slate-200">{{ item[chart.val_key] }}</span>
              </div>
      ...
      <div v-if="dashboard.recent_list && dashboard.recent_list.length" class="bg-slate-900 border border-slate-800 rounded-xl p-5 shadow-sm">
        <div class="flex items-center justify-between mb-4">
          ...
          <span class="text-[11px] text-slate-500 font-mono">{{ dashboard.recent_list.length }} items</span>
        </div>

        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs">
            <thead>
              <tr class="text-slate-400 border-b border-slate-800 uppercase text-[10px] tracking-wider font-semibold">
            ...
            <tbody class="divide-y divide-slate-800/60 text-slate-300 font-mono">
              ...
              >
                <td class="py-2.5 px-3 font-bold text-blue-400">
                  {{ rec.name }}
                </td>
