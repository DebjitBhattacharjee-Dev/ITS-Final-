...
(()=>{
 newDialog=function(el){
  ...
  modal(`<h2>Create ${esc(schema.label)}</h2><p>Select the project, then complete the document details and items.</p><form id="direct-document-form" data-type="${type}" data-test="${esc(test||'')}"><label class="field">Project<select name="project" required><option value="">Select a project…</option>${projects.map(p=>`<option value="${esc(p.id)}" ${p.id===project?'selected':''}>${esc(p.id)} · ${esc(p.name)}</option>`).join('')}</select></label><p id="direct-document-error" role="alert"></p><div class="form-actions"><button type="button" class="button" data-action="close">Cancel</button><button class="button primary">Continue to ${esc(schema.label)} details →</button></div></form>`);
 };
 document.addEventListener('submit',e=>{if(e.target.id!=='direct-document-form')return;e.preventDefault();const form=e.target,project=new FormData(form).get('project'),type=form.dataset.type;try{const r=makeRecord(type,project);r.title=SCHEMAS[type].label;if(type==='inspection'&&form.dataset.test)r.fields.testType=form.dataset.test;records.push(r);closeModal();route='record/'+r.id;recordTab='details';errors=[];editDraft=structuredClone(r);location.hash=route;render();window.scrollTo(0,0);notify('Complete the form, then Save draft or Save & submit.');}catch(err){$('direct-document-error').textContent=err.message;}});
